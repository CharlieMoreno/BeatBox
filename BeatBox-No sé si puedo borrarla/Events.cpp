...
int     tapsCounter;			// Número de Taps registrados hasta el momento. ***SUGERENCIA DE LLAMARLO tapsCounter*** howManyTaps
...
int     compassLenght;			// Duración del compás en ms.
...
long    beatTimes[32];			// Instantes de tiempo absolutos de los beats del metrónomo. Se pueden registrar hasta 32 (sobrao).
int     lastBeatIndex;				// Índice de los beats del metrónomo.
...
void deleteTapsWithInstrument(int instrument) {		// Borra los taps que usen el instrumento nº i, es decir, los taps de la cara 'i' del cubo.
	for(int j = tapsCounter - 1; j >= 0; j--) {		// Para ello, se recorren todos los taps registrados hasta el momento.
		if (taps[j].instrument == instrument) {		// Si un tap j utiliza el instrumento i, 
			if (j != tapsCounter - 1) {				// y si no es el último tap registrado,
				taps[j] = taps[tapsCounter - 1];	// se sobreescribe con el último tap (porque da igual el orden en el array taps[])
			}
			tapsCounter--;							// y se decrementa el número de taps. Si el tap j fuera el último, no se sobreescribe ni se borra, solo se decrementa el nº de taps.
		}
	}
}
